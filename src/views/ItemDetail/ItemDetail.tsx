...
interface IItemDetailProps extends IProps {
    cartStoreService: CartStoreService
}

@inject('cartStoreService')
@observer
export default class ItemDetailScreen extends Component<IItemDetailProps, any> {
    // 解决安卓机标题偏右问题
    ...
    @computed get cartCount() {
        ...
        const { foodList } = this.props.cartStoreService;
    ...
    // 同步更新购物车页面的数据
    updateCartScreen(value: any) {
        const { num } = this.state;
        const { navigation, } = this.props;
        const { value: val } = navigation && navigation.state.params;
        const { name } = val;
        // 判断购物车页面是否存在同样名字的物品
        const itemData = value;
        // 找出索引
        const index = this.props.cartStoreService.foodList.findIndex(e => e.name === name);
        // 不存在
        if (index === -1) {
            // 增加需要用到的字段
            itemData.count = 0;
            itemData.isSelected = true;
            this.props.cartStoreService.foodList.push(value);
            const { length } = this.props.cartStoreService.foodList;
            this.props.cartStoreService.foodList[length - 1].count += num;
        } else {
            // 增加对应name的count
            this.props.cartStoreService.foodList[index].count += num;
        }
    }
    ...
    render() {
        ...
        const { num, bounceValue } = this.state;
        const { navigation } = this.props;
        const { value } = navigation && navigation.state.params;
